...
int main() {
    ...
    TriangleSoup moonSphere;
    moonSphere.createSphere(1.0, 30);
    moonSphere.printInfo();
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        modelViewStack.scale(0.25f);  // Scale unit sphere to radius 0.5
        ...
        modelViewStack.push();
        // Earth
        modelViewStack.rotY(time);            // Earth orbit rotation
        ...
        modelViewStack.rotY(time/2);                 // Earth's rotation around its axis
        // modelViewStack.rotZ(static_cast<float>(-M_PI_2));  // Orient the poles along Y axis instead of Z
        modelViewStack.scale(0.12f);   // Scale unit sphere to radius 0.1
        // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        ...
        // Moon
        modelViewStack.push();
        //modelViewStack.rotY(1.2f * time);
        modelViewStack.translate(0.2f, 0.0f, 0.0f);
        modelViewStack.scale(0.06f);
        modelViewStack.rotY(time);
       // modelViewStack.rotZ(time);
        // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        // Render the geometry to draw the earth
        ...
        modelViewStack.push();
        modelViewStack.rotY(time/3);                   // Earth orbit rotation
        modelViewStack.translate(1.4f, 0.0f, 0.0f);
        modelViewStack.rotY(time);  // Earth's rotation around its axis
        // modelViewStack.rotZ(static_cast<float>(-M_PI_2));  // Orient the poles along Y axis
        // instead of Z
        ...
        modelViewStack.push();
        modelViewStack.rotY(time/2);  // Earth orbit rotation
        modelViewStack.translate(1.4f, 0.0f, 0.0f);
        modelViewStack.rotY(time);  // Earth's rotation around its axis
        // modelViewStack.rotZ(static_cast<float>(-M_PI_2));  // Orient the poles along Y axis
        // instead of Z
